...
import { ChevronLeft, ChevronRight, Play, Music } from "../lib/icons";
...
export function RecentlyPlayed({ onAlbumClick }: RecentlyPlayedProps) {
  ...
  const fetchAlbums = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await subsonicService.getAlbumList2(
        "recent",
        ALBUMS_PER_PAGE,
        offset,
      );
      ...
      if (!data || data.length === 0) {
        ...
        setAlbums((prev) => {
          ...
          const uniqueNewAlbums = data.filter(
    ...
    return (
      <div className="w-full mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl md:text-2xl font-bold retro">Recently Played</h2>
        </div>
          ...
          >
            ...
            >
              ...
              <div className="relative aspect-square mb-3 border-2 border-foreground/20 dark:border-ring/20 overflow-hidden bg-zinc-800">
                ...
                <Music
                  className="absolute top-2 right-2 w-5 h-5 text-white/80
