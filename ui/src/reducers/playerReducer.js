...
const reduceAddTracks = (state, { data }) => {
  ...
  Object.keys(data).forEach((id) => {
    if (!queue.find((item) => item.trackId === data[id].id)) {
...
const reduceSyncQueue = (
  state,
  { data: { audioInfo, audioLists, playIndex } },
) => {
  return {
    ...state,
    queue: audioLists,
    clear: false,
    playIndex: playIndex !== undefined ? playIndex : state.playIndex,
  }
}
...
const reduceSetPlayIndex = (state, { data: { playIndex } }) => {
  return {
    ...state,
    playIndex,
    clear: true,
...
export const playerReducer = (previousState = initialState, payload) => {
  ...
  switch (type) {
    ...
    case PLAYER_SET_PLAY_INDEX:
      return reduceSetPlayIndex(previousState, payload)
    default:
